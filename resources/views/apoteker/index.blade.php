...
@section('title', 'Kelola Apoteker')

@section('page-title', 'Kelola Apoteker')
...
<div class="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0 mb-6 sm:mb-8">
    <div>
        ...
        <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400">
            Kelola data Apoteker dalam sistem
        </p>
    </div>
...
<div class="flex flex-col sm:flex-row sm:items-end sm:space-x-4 space-y-4 sm:space-y-0 justify-between">
    ...
    <div class="sm:w-2/4">
        <div class="relative">
            ...
            <input type="text" id="user-search" placeholder="Cari nama, username, atau kode..."
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700" id="users-table-body">
                @forelse ($users as $user)
                    ...
                    <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap sticky left-8 bg-white dark:bg-gray-800 z-10 sm:static">
                        ...
                        <div class="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-purple-500 to-purple-600 rounded-full flex items-center justify-center text-white font-semibold text-xs sm:text-sm flex-shrink-0">
                            ...
                            <div class="ml-2 sm:ml-4 min-w-0">
                                <div class="text-xs sm:text-sm font-medium text-gray-900 dark:text-white truncate">{{ $user->name }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap hidden sm:table-cell">
                    ...
                    <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap">
                        <div class="text-xs sm:text-sm text-gray-900 dark:text-white">{{ $user->alamat }}</div>
                    </td>
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-8">
                            <div class="text-gray-500 dark:text-gray-400">
                                ...
                                <p class="text-lg font-medium">Data apoteker tidak tersedia</p>
                                <p class="text-sm">Belum ada apoteker yang terdaftar dalam sistem</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    <div id="card-view" class="hidden">
        ...
        @forelse ($users as $user)
            <div class="user-card bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-gray-900 dark:text-white">{{ $user->alamat }}</span>
                    </div>
                </div>
            </div>
            @empty
            <div class="text-center py-8">
                <div class="text-gray-500 dark:text-gray-400">
                    <i class="fas fa-users text-4xl mb-4"></i>
                    <h4 class="text-lg font-medium">Data Apoteker Tidak Tersedia</h4>
                    <p class="text-sm">Belum ada apoteker yang terdaftar dalam sistem</p>
                </div>
            </div>
            @endforelse
...
<!-- Create User Modal -->
<div id="create-user-modal" class="fixed inset-0 bg-black/50 dark:bg-black/70 z-50 hidden backdrop-blur-sm transition-all duration-300">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-sm sm:max-w-md transform transition-all duration-300 scale-95 max-h-[90vh] overflow-y-auto">
            <div class="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 z-10">
                <div class="flex items-center justify-between">
                    <h3 class="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">
                        Tambah Apoteker Baru
                    </h3>
                    <button onclick="closeCreateUserModal()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-1">
            ...
            <form id="create-user-form" class="p-4 sm:p-6">
                ...
                <div class="space-y-3 sm:space-y-4">
                    ...
                    <!-- Password -->
                    <div>
                        <label for="create-password" class="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    ...
                    <!-- Confirm Password -->
                    <div>
                        <label for="create-password-confirm" class="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
...
<!-- Edit User Modal -->
<div id="edit-user-modal" class="fixed inset-0 bg-black/50 dark:bg-black/70 z-50 hidden backdrop-blur-sm transition-all duration-300">
    <div class="flex items-center justify-center min-h-screen p-2 sm:p-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-sm sm:max-w-md transform transition-all duration-300 scale-95 max-h-[90vh] overflow-y-auto">
            <div class="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 z-10">
                <div class="flex items-center justify-between">
                    ...
                    <button onclick="closeEditUserModal()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-1">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
            </div>
            ...
            <form id="edit-user-form" class="p-4 sm:p-6">
                ...
                <div class="space-y-3 sm:space-y-4">
                    ...
                    <!-- Username -->
                    <div>
                        <label for="edit-username" class="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Username
                        </label>
...
function updateUser() {
    ...
    // Set form action and method
    form.action = `/admin/apoteker/${userId}`;
    form.method = 'POST';
...
function deleteUser(userId, userName) {
    ...
    if (!confirm(`Apakah Anda yakin ingin menghapus Apoteker "${userName}"? Data yang dihapus tidak dapat dikembalikan!`)) {
        return;
    }
