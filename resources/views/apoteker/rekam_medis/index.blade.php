            ...
            <table class="w-full text-left min-w-full">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-700">
                        ...
                        <th class="px-4 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Validasi</th>
                        <!-- ✅ Baru -->
                    ...
                    @forelse ($dataRekamMedis as $item)
                        <tr class="rekam-row">
                            ...
                            <td class="text-sm text-gray-900 dark:text-white">
                                @if ($item->status === 'diobati')
                                    <button
                                        class="bg-green-600 text-white font-semibold px-3 py-1 rounded cursor-not-allowed"
                                        disabled>
                                        Tervalidasi
                                    </button>
                                @else
                                    <form action="{{ route('apoteker.rekam-medis.validasi', $item->id) }}" method="POST"
                                        onsubmit="return confirmValidasi(event)">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                            class="bg-gray-400 text-white font-semibold px-3 py-1 rounded hover:bg-gray-500">
                                            VALIDASI
                                        </button>
                                    </form>
                                @endif
                            ...
                            <td class="px-4 py-3 text-sm text-right">
                                <div class="flex justify-end space-x-2">
                                    ...
                                    <form action="{{ route('apoteker.rekammedis.destroy', $item->id) }}" method="POST"
    ...
    <div id="create-rekammedis-modal"
        ...
        <div class="flex items-center justify-center min-h-screen p-2 sm:p-4">
            <div
                ...
                <form id="create-rekammedis-form" action="{{ route('apoteker.rekammedis.store') }}" method="POST"
                    class="p-4 sm:p-6">
                    @csrf
                    <div class="space-y-4">
...
@push('scripts')
    ...
    function confirmValidasi(event) {
        ...
        function editRekamMedis(id) {
            ...
            // Set form action
            document.getElementById('edit-rekammedis-form').action = `/apoteker/rekammedis/${id}`;
