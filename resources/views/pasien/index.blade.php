...
<div class="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0 mb-6 sm:mb-8">
    <div>
        ...
        <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400">Kelola data pendaftaran dan identitas pasien.</p>
    </div>
        ...
        <table class="w-full text-left min-w-full">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <div class="text-sm font-bold text-purple-600 dark:text-purple-400">{{ $pasien->kode_pasien }}</div>
                        <div class="text-xs text-gray-500">{{ $pasien->identity->identity_number }}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            <div class="w-8 h-8 bg-purple-100 text-purple-600 rounded-full flex items-center justify-center font-bold mr-3">
                                {{ strtoupper(substr($pasien->identity->name, 0, 1)) }}
                            </div>
                            <div>
                                <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $pasien->identity->name }}</div>
                    ...
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 text-xs rounded-full {{ $pasien->status == 'terdaftar' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                    ...
                    <td class="px-6 py-4 text-right space-x-2">
                        ...
                        <a href="{{ route('admin.pasien.show', $pasien->id) }}" class="text-blue-600 hover:text-blue-900"><i class="fas fa-eye"></i></a>
                        
                        <form action="{{ route('admin.pasien.destroy', $pasien->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data ini?')">
                            @csrf @method('DELETE')
...
<div id="create-pasien-modal" class="fixed inset-0 bg-black/50 z-50 hidden backdrop-blur-sm">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-lg overflow-hidden">
            ...
            <form id="create-pasien-form" action="{{ route('admin.pasien.store') }}" method="POST" class="p-6 space-y-4">
                @csrf
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">No. Identitas (NIM/NIDN/NIP) <span class="text-red-500">*</span></label>
                        ...
                        <p class="text-[10px] mt-1 text-gray-500 italic">*Jika sudah pernah terdaftar, data akan otomatis terisi.</p>
                    </div>

                    <div>
                        <label class="block text-sm font-medium dark:text-gray-300">Kategori</label>
                        <select name="identity_type" id="create-identity-type" class="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:text-white">
                    ...
                    <div>
                        ...
                        <select name="gender" id="create-gender" class="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:text-white">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="nama_pasien" id="create-name" required class="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:text-white">
                    </div>

                    <div class="sm:col-span-2">
                        <label class="block text-sm font-medium dark:text-gray-300">Pilih Poli</label>
                        <select name="poli" class="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:text-white">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="alamat" id="create-address" rows="2" class="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:text-white"></textarea>
                    </div>
                </div>

                <div class="flex justify-end space-x-3 mt-6">
                    <button type="button" onclick="closeCreatePasienModal()" class="px-4 py-2 text-gray-600">Batal</button>
                    <button type="submit" class="px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">Simpan Pendaftaran</button>
                </div>
            </form>
        </div>
    </div>
            ...
            fetch(`/admin/pasien/identity/${val}`)
                ...
                .then(data => {
                    ...
                    if (data) {
                        ...
                        setTimeout(() => document.getElementById('create-name').classList.remove('bg-green-50'), 1000);
                    }
                })
