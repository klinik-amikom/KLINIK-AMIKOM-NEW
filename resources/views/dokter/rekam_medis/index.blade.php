        ...
        <table class="w-full text-left min-w-full">
            <thead>
                <tr class="bg-gray-50 dark:bg-gray-700">
                    ...
                    <th class="px-4 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Tanggal Periksa</th>
                    ...
                    <th class="px-4 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Tgl Ambil Obat</th>
                    <th class="px-4 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Jumlah Obat</th>
                    <th class="px-4 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase text-right">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="rekam-row">
                    ...
                    <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">{{ $item->pasien->nama_pasien ?? '-' }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">{{ $item->user->name ?? '-' }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">{{ $item->obat->nama_obat ?? '-' }}</td>
                    ...
                    <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">{{ $item->tanggal_pengambilan ?? '-' }}</td>
                    <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">{{ $item->jumlah_obat ?? '0' }}</td>
                    <td class="px-4 py-3 text-sm text-right">
                        <div class="flex justify-end space-x-2">
                            ...
                            <form action="{{ route('dokter.rekammedis.destroy', $item->id) }}" method="POST" class="inline"
...
<div class="flex justify-end mt-4">
    <a href="{{ route('dokter.rekam-medis.export.pdf') }}"
       class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 mr-2">
        Export PDF
    </a>
    <a href="{{ route('dokter.rekam-medis.export.excel') }}"
...
<div id="create-rekammedis-modal" class="fixed inset-0 bg-black/50 dark:bg-black/70 z-[999] hidden backdrop-blur-sm transition-all duration-300">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-sm sm:max-w-md transform transition-all duration-300 scale-95 max-h-[90vh] overflow-y-auto">
            <div class="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 z-10">
                <div class="flex items-center justify-between">
                    ...
                    <button onclick="closeCreateRekamMedisModal()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-1">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
            </div>

            <form id="create-rekammedis-form" action="{{ route('dokter.rekammedis.store') }}" method="POST" class="p-4 sm:p-6">
                @csrf
                <div class="space-y-4">

                    <!-- Tanggal Periksa -->
                    <div>
                        <label for="tanggal_periksa" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    ...
                    <!-- Tanggal Ambil Obat -->
                    <div>
                        <label for="tgl_ambil_obat" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Tanggal Ambil Obat
                        </label>
                    ...
                    <!-- Jumlah Obat -->
                    <div>
                        <label for="jumlah_obat" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Jumlah Obat
                        </label>
                ...
                <div class="px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-200 dark:border-gray-700 rounded-b-xl mt-6">
...
<div id="edit-rekammedis-modal" class="fixed inset-0 bg-black/50 dark:bg-black/70 z-[999] hidden backdrop-blur-sm transition-all duration-300">
    <div class="flex items-center justify-center min-h-screen p-2 sm:p-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 w-full max-w-sm sm:max-w-md transform transition-all duration-300 scale-95 max-h-[90vh] overflow-y-auto">
            <div class="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 z-10">
                <div class="flex items-center justify-between">
                    ...
                    <button onclick="closeEditRekamMedisModal()" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-1">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
            </div>
            ...
            <form id="edit-rekammedis-form" method="POST" class="p-4 sm:p-6">
                ...
                <div class="space-y-4">
                    ...
                    <!-- Tanggal Periksa -->
                    <div>
                        <label for="edit_tanggal_periksa" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Tanggal Periksa <span class="text-red-500">*</span>
                        </label>
                    ...
                    <!-- Pasien -->
                    <div>
                        <label for="edit_pasien_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Pasien <span class="text-red-500">*</span>
                        </label>
                    ...
                    <!-- Dokter -->
                    <div>
                        <label for="edit_dokter_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Dokter <span class="text-red-500">*</span>
                        </label>
                    ...
                    <!-- Obat -->
                    <div>
                        <label for="edit_obat_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Obat <span class="text-red-500">*</span>
                        </label>
                    ...
                    <!-- Diagnosis -->
                    <div>
                        <label for="edit_diagnosis" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Diagnosis
                        </label>
                    ...
                    <!-- Tanggal Ambil Obat -->
                    <div>
                        <label for="edit_tgl_ambil_obat" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Tanggal Ambil Obat
                        </label>
                    ...
                    <!-- Jumlah Obat -->
                    <div>
                        <label for="edit_jumlah_obat" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                            Jumlah Obat
                        </label>
...
<script>
    ...
    function editRekamMedis(id) {
        ...
        // Set form action
        document.getElementById('edit-rekammedis-form').action = `/dokter/rekammedis/${id}`;
